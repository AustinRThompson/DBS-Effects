...
speakerList <- rio::import(file = "Prepped Data/Speaker List.csv") |>
  dplyr::select(Speaker, Sex) |>
  unique()

# Perceptual Ratings ----
data_listenerRatings <- rio::import(file = "Prepped Data/Listener Ratings.csv") |>
...
inter_Off <- data_interListener$Off %>%
  ...
  dplyr::filter(type == "ICC3k") # ICC3k == Consistency
...
inter_Rel <- rbind(inter_Off %>%
  ...
  dplyr::mutate(
    p.value = ifelse(p < .001, "< .001",paste0("",numformat(p, digits = 3))),
    ci.range = paste0(round(.$`lower bound`, digits = 2),
  ...
  dplyr::select(Dimension, DBS_Status, Interrater, ICC, ci.range, p.value) %>%
  pivot_wider(
    names_from = DBS_Status,
    values_from = c(Interrater, ICC, ci.range, p.value)
  ) %>%
  dplyr::filter(Dimension != "LE") %>%
...
# Tables
## Table 3 - Interrater Listener Reliability
```{r}
inter_Rel %>%
  dplyr::select(!c(Interrater_Off, Interrater_On)) %>%
  gt() %>%
  gt::tab_spanner(
    label = "Off",
    columns = c(ICC_Off, ci.range_Off, p.value_Off)
  ) %>%
  gt::tab_spanner(
    label = "On",
    columns = c(ICC_On, ci.range_On, p.value_On)
  ) %>%
  gt::cols_label(
    `ICC_Off` = "ICC",
    `ICC_On` = "ICC",
  ...
  gt::cols_align(
    align = c("right"),
    columns = c(p.value_Off, p.value_On)) %>%
  gt::cols_align(
    align = c("left"),
    columns = c(Dimension)) %>%
  gt::fmt_number(
    columns = c(ICC_Off, ICC_On),
    decimals = 2
  ) %>%
  gt::gtsave(
    filename = "Tables/Table 3_Interrater Reliability.html"
  )

```

## Table 4 - Descriptive Perceptual Measures
```{r}
SpeakerSex <- rio::import("Prepped Data/Speaker List.csv") %>%
...
tableData <- rbind(data_listenerRatings,
  ...
  gt::gtsave(filename = "Tables/Table 4_Perceptual Descriptives.html")

```

## Table 5 - Perceptual Pairwise Table
```{r}
...
pairwiseTable_t(pairs(emms)) %>%
  ...
  gt::gtsave(filename = "Table 5_Perceptual Pairwise Comparisons.html",
             path = "Tables")
```

## Table 6 - Descriptive Acoustic Measures
```{r}
...
tableData <- rbind(combinedData, combinedData %>%
  ...
  gt::gtsave(filename = "Tables/Table 6_Acoustic Descriptives.html")
...
sjPlot::tab_model(
  ...
  dv.labels = c("Voicing Proportion (%)",
                ...
                "F0 Variation (Semitone)",
                "Intensity Variation (dB)"),
  file = "Tables/Table 7_Acoustic Model Table.html"
)
```
